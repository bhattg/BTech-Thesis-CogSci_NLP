...
##########################################################
    ...
    #         #tuple of list 
            ...
            test_data_dict[files] = pickel.load(test_pickel)
    ...
    def external_testing(self,d=None):
        ...
        for files in d.keys():
            ...
            print(files + " "+str(np.sum(np.asarray(Y_testing_perFile)==np.asarray(predicted))/len_X_testing)+" "+str(len_X_testing))
        return testing_result
    
    def load_external_testing(self, filename, save_processed_data = True):
        ex_list = []
        for files in os.listdir(filename):
            pickel = pickle.load(open(os.path.join(root, files), 'rb'))
