...
# self.update_dump_dict(keys, x_test_minibatch, y_test_minibatch, predicted)
    ...
    def create_model_batched(self, batch_size=32):
        ...
        self.model = LSTM(input_units = self.maxlen ,hidden_units = self.hidden_dim, vocab_size = len(self.vocab_to_ints)+1, batch_size=self.batch_size, embedding_dim=self.embedding_size, num_layers=2).to(device)
    ...
    def results_batched(self):
        ...
        with torch.no_grad():
            for index in range(len(x_test)) :
                ...
                pred = pred.view(batch_size_testing,2)
                if (pred[0][0] > pred[0][1]) :
                    output = 0
        ...
        acc = sum(accuracy)/len(x_test)
        self.log("Accuracy = {}".format(acc))
        return acc
