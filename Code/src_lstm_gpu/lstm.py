...
class LstmModule(nn.Module):
    def __init__(self, input_units, output_units, hidden_units, batch_size, bias = True, num_chunks = 4, embedding_dim = 50):
        ...
        self.bias = bias
        self.weight_ih = nn.Parameter(torch.Tensor(batch_size, num_chunks * hidden_size, input_size).cuda())
        self.weight_hh = nn.Parameter(torch.Tensor(batch_size, num_chunks * hidden_size, hidden_size).cuda())
        if bias:
            self.bias_ih = nn.Parameter(torch.Tensor(batch_size, num_chunks * hidden_size).cuda())
            self.bias_hh = nn.Parameter(torch.Tensor(batch_size, num_chunks * hidden_size).cuda())
        else:
...
class LSTM(nn.Module):
    ...
    def forward(self, input_, max_time = 50) :
        ...
        allh = torch.Tensor(all_hidden).cuda().transpose(0, 1)
        allo = torch.Tensor(all_outputs).cuda().transpose(0, 1)
        alli = torch.Tensor(all_i).cuda().transpose(0, 1)
        allf = torch.Tensor(all_f).cuda().transpose(0, 1)
        allg = torch.Tensor(all_g).cuda().transpose(0, 1)
        allc = torch.Tensor(all_cprev).cuda().transpose(0, 1)
