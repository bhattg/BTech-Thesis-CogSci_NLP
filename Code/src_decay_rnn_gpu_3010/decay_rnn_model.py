...
class DECAY_RNN_Model(object):
    ...
    def train(self, n_epochs=10, model_prefix='__'):
        ...
        x_train = torch.tensor(self.X_train, dtype=torch.long, requires_grad=False).cuda()
        ...
        for epoch in range(n_epochs) :
            ...
            for index in range(fffstart, len(x_train)) :
                ...
                if (y_train[index] == 0) :
                    actual = torch.autograd.Variable(torch.tensor([0]), requires_grad=False).cuda()
                else :
                    actual = torch.autograd.Variable(torch.tensor([1]), requires_grad=False).cuda()
    ...
    def train_batched(self, n_epochs=10, model_prefix="__", batch_size=32, shuffle=True, learning_rate=0.1, num_workers=0):
        ...
        '''
        total_batches = int(len(self.X_train)/self.batch_size)
