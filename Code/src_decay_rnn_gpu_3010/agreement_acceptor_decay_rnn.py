...
class RNNAcceptor(DECAY_RNN_Model):
    ...
    def create_model_batched(self, batch_size=32):
        ...
        self.model = LSTM(input_units = self.maxlen ,hidden_units = self.hidden_dim, vocab_size = len(self.vocab_to_ints)+1, batch_size=self.batch_size,embedding_dim=self.embedding_size).cuda()


    def create_model(self):
        self.log('Creating model')
        self.log('vocab size : ' + str(len(self.vocab_to_ints)))
        self.model = LSTM(input_units = self.maxlen ,hidden_units = self.hidden_dim, vocab_size = len(self.vocab_to_ints)+1, embedding_dim=self.embedding_size).cuda()
    ...
    def results_train(self):
        ...
        predicted = []
        x_train = torch.tensor(self.X_train, dtype=torch.long).cuda()
