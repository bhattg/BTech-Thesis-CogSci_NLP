...
class RNNAcceptor(JOINT_MODEL):
    ...
    def create_model(self):
        ...
        self.log('vocab size : ' + str(len(self.vocab_to_ints)))
        self.model_lstm = LSTM(input_units = self.maxlen ,hidden_units = self.hidden_dim, vocab_size = len(self.vocab_to_ints)+1)#.to(device)
        self.model_decay = LSTM_D(input_units = self.maxlen ,hidden_units = self.hidden_dim, vocab_size = len(self.vocab_to_ints)+1)
    ...
    def results_decay(self):
        ...
        with torch.no_grad():
            for index in range(len(x_test)) :
                pred, hidden, output , _= self.model_decay(x_test[index])
                if (pred[0][0] > pred[0][1]) :
                    predicted.append([0])
                else :
                    predicted.append([1])
    ...
    def results_lstm(self):
        self.log_LSTM('Processing test set')
        predicted = []
        x_test = torch.tensor(self.X_test, dtype=torch.long)#.to(cpu)
        # x_test = self.X_test
        self.log_LSTM(str(len(self.X_train)) + ', ' + str(len(x_test)))

        with torch.no_grad():
            for index in range(len(x_test)) :
                pred, hidden, output, _,_ = self.model_lstm(x_test[index])
                if (pred[0][0] > pred[0][1]) :
                    predicted.append([0])
                else :
                    predicted.append([1])
        ...
        self.test_results_lstm = pd.DataFrame(recs, columns=columns)
        xxx = self.test_results_lstm['correct']
        self.log_LSTM('Accuracy : ' + str(sum(xxx)))
        return sum(xxx)
