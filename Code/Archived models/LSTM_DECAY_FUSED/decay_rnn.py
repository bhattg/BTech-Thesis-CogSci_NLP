...
class LstmModule(nn.Module):
    ...
    def __init__(self, input_units, output_units, hidden_units, batch_size, bias = True, num_chunks = 1, embedding_dim = 50):
        ...
        self.rgate = nn.Parameter(torch.tensor(1.0))
...
class LSTM_D(nn.Module):
    ...
    # To compare the gate value and the alpha value we have to do this. 
    def get_rgate_vector(self):
        for name,param in self.named_parameters():
            if(name=="cell_0.rgate"):
                return param.item()*np.ones(self.hidden_units)
    ...
    def forward(self, input_, max_time = 50) :
        ...
        return softmax_out, all_hidden, all_outputs, self.get_rgate_vector()
